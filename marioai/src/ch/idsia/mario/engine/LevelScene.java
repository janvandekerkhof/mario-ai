...
public class LevelScene extends Scene implements SpriteContext
{
    ...
    public LevelScene(GraphicsConfiguration graphicsConfiguration, MarioComponent renderer, Level level, int timeLimit){
        this(graphicsConfiguration, renderer, 0, 0, 0, 0, timeLimit);
        this.level = level;
    }
...
//        level = LevelGenerator.createLevel(levelLength, 15, levelSeed, levelDifficulty, levelType);
        ...
        mario = new Mario(this, getMarioYStart(level) -1);
        sprites.add(mario);
        startTime = 1;
    ...
    public void init()
    {
        ...
        /*        if (replayer!=null)
         ...
         {*/
//        level = LevelGenerator.createLevel(320, 15, levelSeed);
        ArrayList<Level> levels = LevelGenerator.createLevelSegments(100, levelSeed, levelDifficulty, levelType, 22);
        level = levels.get(0);
//        level = LevelGenerator.createLevel(levelLength, 15, levelSeed, levelDifficulty, levelType);
        //        }
        ...
        mario = new Mario(this, getMarioYStart(level));
        sprites.add(mario);
        startTime = 1;
    ...
    private int getMarioYStart(Level level){
        for (int j = level.height - 1; j > 0; j--) {
                if(level.getBlock(2, j) == 0)
                {
                    return j + 22 - level.height  ;
                }
        }
        return 0;
    }
